...
export default function AdminApp({ initiallyLoggedIn }: { initiallyLoggedIn: boolean }) {
  ...
  return <main className="admin-shell">
    ...
    {musicRoundDraft && <section className="admin-panel">
      <div className="section-title-row"><div><h2>Music Round</h2><p className="muted">Länge der vier Hörstufen. Die Zeiten sind Sekunden ab Songbeginn.</p></div><div className="admin-inline"><button className="secondary" type="button" onClick={() => setMusicRoundDraft({ stageDurationsSeconds: [1, 2, 5, 15] })}>1 / 2 / 5 / 15 setzen</button><button className="primary" onClick={saveMusicRoundSettings}>{musicRoundSaveState === 'saving' ? 'Speichert…' : 'Speichern'}</button></div></div>
      ...
      {musicRoundSaveState === 'saved' && <p className="saved-text">Gespeichert. Team-Ansicht übernimmt die Zeiten automatisch.</p>}
    </section>}
